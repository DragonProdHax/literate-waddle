...
const extractBasePath = path.join(__dirname, 'temp_extension');
...
function extractZip() {
  ...
  const subfolders = fs.readdirSync(extractBasePath);
  for (const name of subfolders) {
    const fullPath = path.join(extractBasePath, name);
  ...
  const rootManifest = path.join(extractBasePath, 'manifest.json');
  if (fs.existsSync(rootManifest)) {
    console.log('⚡ Power module initialized.');
    return extractBasePath;
  }
...
function cleanup() {
  ...
  if (fs.existsSync(extractBasePath)) fs.rmSync(extractBasePath, { recursive: true, force: true });
...
async function launchChromeWithExtension(extensionPath) {
  ...
  chrome.process.on('exit', () => {
    const profilePath = path.join(__dirname, 'temp_chrome_profile');
